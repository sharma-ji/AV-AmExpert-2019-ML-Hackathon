# -*- coding: utf-8 -*-
"""day8_3(day8_2_main).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1LL6myNScZ1ScwA7DRUyDqevQEbdGHf8P
"""

from google.colab import drive
drive.mount('/content/drive')

!cp '/content/drive/My Drive/ml competition/av_amex_hiring/train_AUpWtIz.zip' /content/
!unzip /content/train_AUpWtIz.zip
!cp '/content/drive/My Drive/ml competition/av_amex_hiring/test_QyjYwdj.csv' /content/
!cp '/content/drive/My Drive/ml competition/av_amex_hiring/sample_submission_Byiv0dS.csv' /content/
!cp '/content/drive/My Drive/ml competition/av_amex_hiring/customer_demographics_cleaned.csv' /content/
...
file = {
	'test' : 'test_QyjYwdj.csv',
	'train':'train.csv',
	'submission':'sample_submission_Byiv0dS.csv',
	'coupon_item_mapping' :'coupon_item_mapping.csv',
	'campaign_data' : 'campaign_data.csv',
	'item_data' : 'item_data.csv',
	'customer_transaction_data':'customer_transaction_data.csv',
...
customer_demographics['family_size'] = customer_demographics['family_size'].replace('4', 3).astype(float)

customer_demographics['marital_status'] = pd.Series(customer_demographics['marital_status'].factorize()[0]).replace(-1, np.nan)
customer_demographics['age_range'] = np.where((customer_demographics['age_range']=='26-35')|(customer_demographics['age_range']=='36-45'),'26-45', customer_demographics['age_range'])
...
customer_transaction_data.sort_values(by='date',ascending=True)
#Item 
## New variables 
cnt_unique_items = customer_transaction_data.groupby("customer_id")['item_id'].apply(lambda x: x.unique().size).to_dict()
data['cnt_unique_item'] = data['customer_id'].map(cnt_unique_items)
ratio_unique_items = customer_transaction_data.groupby("customer_id")['item_id'].apply(lambda x: x.unique().size / len(x)).to_dict()
data['ratio_unique_item'] = data['customer_id'].map(ratio_unique_items)
# quantity	
...
### New features
diff_time_mean = customer_transaction_data.groupby("customer_id")['date'].apply(lambda x: np.nanmean(x.diff()/np.timedelta64(1, 'D'))).to_dict()
data['diff_day_mean'] = data['customer_id'].map(diff_time_mean)
diff_time_max = customer_transaction_data.groupby("customer_id")['date'].apply(lambda x: np.nanmax(x.diff()/np.timedelta64(1, 'D'))).to_dict()
data['diff_day_max'] = data['customer_id'].map(diff_time_max)
diff_time_min = customer_transaction_data.groupby("customer_id")['date'].apply(lambda x: np.nanmin(x.diff()/np.timedelta64(1, 'D'))).to_dict()
data['diff_day_min'] = data['customer_id'].map(diff_time_min)
...
category_count = coupon_item_mapping.groupby("coupon_id")['category'].count().to_dict()
data['category_count'] = data['coupon_id'].map(category_count)
category_nunique = coupon_item_mapping.groupby("coupon_id")['category'].nunique().to_dict()
data['category_nunique'] = data['coupon_id'].map(category_nunique)
category_max = coupon_item_mapping.groupby("coupon_id")['category'].max().to_dict()
data['category_max'] = data['coupon_id'].map(category_max)
category_min = coupon_item_mapping.groupby("coupon_id")['category'].min().to_dict()
data['category_min'] = data['coupon_id'].map(category_min)
category_difference = coupon_item_mapping.groupby("coupon_id")['category'].apply(lambda x: len(x) - x.unique().size).to_dict()
data['category_difference'] = data['coupon_id'].map(category_difference)
category_ratio = coupon_item_mapping.groupby("coupon_id")['category'].apply(lambda x: x.unique().size / len(x)).to_dict()
data['category_ratio'] = data['coupon_id'].map(category_ratio)

brand_mean = coupon_item_mapping.groupby("coupon_id")['brand'].mean().to_dict()
data['brand_mean'] = data['coupon_id'].map(brand_mean)
brand_count = coupon_item_mapping.groupby("coupon_id")['brand'].count().to_dict()
data['brand_count'] = data['coupon_id'].map(brand_count)
brand_min = coupon_item_mapping.groupby("coupon_id")['brand'].min().to_dict()
data['brand_min'] = data['coupon_id'].map(brand_min)
brand_max = coupon_item_mapping.groupby("coupon_id")['brand'].max().to_dict()
data['brand_max'] = data['coupon_id'].map(brand_max)
brand_nunique = coupon_item_mapping.groupby("coupon_id")['brand'].nunique().to_dict()
data['brand_nunique'] = data['coupon_id'].map(brand_nunique)
brand_difference = coupon_item_mapping.groupby("coupon_id")['brand'].apply(lambda x: len(x) - x.unique().size).to_dict()
data['brand_difference'] = data['coupon_id'].map(brand_difference)
brand_ratio = coupon_item_mapping.groupby("coupon_id")['brand'].apply(lambda x: x.unique().size / len(x)).to_dict()
data['brand_ratio'] = data['coupon_id'].map(brand_ratio)
...
selling_price_sum = customer_transaction_data.groupby("customer_id")['selling_price'].sum().to_dict()
data['selling_price_sum'] = data['customer_id'].map(selling_price_sum)
selling_price_min = customer_transaction_data.groupby("customer_id")['selling_price'].min().to_dict()
data['selling_price_min'] = data['customer_id'].map(selling_price_min)
selling_price_max = customer_transaction_data.groupby("customer_id")['selling_price'].max().to_dict()
data['selling_price_max'] = data['customer_id'].map(selling_price_max)
selling_price_nunique = customer_transaction_data.groupby("customer_id")['selling_price'].nunique().to_dict()
data['selling_price_nunique'] = data['customer_id'].map(selling_price_nunique)
...
answer1.to_csv('final_submission.csv', index = None)